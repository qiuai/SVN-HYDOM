...
function tsF(root){
   //提示内容
	//获取提示数据
	 //data = getTips(jQ);
	//动态修改提示内容
    var tsObject = jQuery('div[class="popover fade bottom in"]').find('div[id^="提示内容_tab_"]');
    var div = '';
       div += '<div class="btn-group mathquill-space btn btn-default"><a class="mathquill-rendered-math2" id="ts1" title="-\\left(-4x-5\\right)-\\sqrt{\\left(x-3\\right)^2}"><span >1、$-\\left(-4x-5\\right)-\\sqrt{\\left(x-3\\right)^2}$</span></a></div>'

    tsObject.html(div);
    MathJax.Hub.Queue(["Typeset",MathJax.Hub]);
    //绑定点击事件
    tsObject.find('a.mathquill-rendered-math2').mousedown(function(e) {
	  	e.stopPropagation();
	}).click(function(){
		//整行替换
		//root.jQ.find(".textarea").siblings().detach();
		var blockId = root.jQ.attr(mqBlockId);
        var block = MathElement[blockId];
		//while(block.ends["1"]!=0){
		var ids = [];
		 block.eachChild(function(uncle){ids.push(uncle.id)})		  
		 	root.jQ.focus();
 		root.cursor.writeLatex(this.title);
		//清楚以前的数据
		block.eachChild(function(uncle){
...
//快捷键提示信息
function writeTsLatex(root,title){
   	//整行替换
		//root.jQ.find(".textarea").siblings().detach();
		var blockId = root.jQ.attr(mqBlockId);
        var block = MathElement[blockId];
...
var RootMathBlock = P(MathBlock, function(_, _super) {
  ...
  _.onKey = function(key, e) {
	...
	case 'Alt-1':
	      var tsObject = jQuery('div[class="popover fade bottom in"]').find('div[id^="提示内容_tab_"]');
		  var title = tsObject.find("a[id=ts1]")[0].title;
		  writeTsLatex(root,title);
	      break;
    ...
    // Prevent newlines from showing up
     case 'Enter': 
    	var jj = $(this.jQ).next().next();
    	$(this.jQ).popover("hide");

    	//jj.focus();
    	//jj.popover("show");
    	//最后一排 回车增加一行
    	if(jj.length==0){
    		var id = $(this.jQ).attr("id").split("_")
    		var pid ="";
    		for(var i=0;i<id.length;i++){
    			if(i==id.length-1){
    				 var nu = parseInt(id[i]) + 1;
    				 pid += nu;
    			}else{
    				pid += id[i] + "_";
    			}
    		}
    		var jQ2 = $(this.jQ).after("<span class='mathquill-editor' style='padding-top:15px;height:45px;width:400px;' id='2'></span>");
			//root = RootMathBlock();
			//root2.jQ = jQ2.attr(mqBlockId, root2.id);
    		//$(this.jQ).parent().prev().children().last().after(newRowHtml);
    		$("#2").mathquill('editor');
			//root2.jQ = $("#2")
			//root2.cursor = Cursor(root2);
    		$("#2").focus();
            $("#2").popover("show");
    	}
    	 break;
...
function addToolbar(root, jQ) {
  ...
  var button_tabs = [
	...
	{name: '基础符号',
      example: '+',
      button_groups: [
        ["therefore", "because", "bigtriangleup", "pi"],
        ["cup", "cap"],
        ["frac", "binomial", "sqrt", "nthroot"],
        [ "delta", "epsilon", "eta"],
        [ "f", "vector"],
        ["subscript", "supscript"], 
        ["alpha", "beta", "gamma"],
        ...
        ["times", "div", "ast", "pm", "mp"], 
        ["cdot", "deg", "prime"],
        ["opencurlybrace", "closecurlybrace", "Delta", "Omega"]
      ]},
      { name: '关系符号',
          example: '&le;',
          button_groups: [
            ["parallel", "angle", "diamondsuit"],
    	    ["cong", "sim", "propto", "approx", "simeq", "bot"], 
            ["ne", "le", "ge"], 
            ["subset", "supset", "notsubset",  "notsupset", "subseteq", "supseteq", "notsubseteq", "notsupseteq"],
            ["in", "ni", "notin", "notni"],
//    	    ["equiv", "models", "prec", "succ", "preceq", "succeq", "mid", "ll", "gg", "parallel", "bowtie", "sqsubset", "sqsupset", "smile", "sqsubseteq", "sqsupseteq", "doteq", "frown", "vdash", "dashv", "exists", "varnothing"]
	  ]},
	  {name: '希腊字符',
	      example: '&pi;',
	      button_groups: [
	        ...
	        ["digamma", "varepsilon", "vartheta", "varkappa"], 
	        ["varpi", "varrho", "varsigma", "varphi"],
	        ["Gamma", "Theta", "Lambda", "Xi"], 
      ...
      { name: '其他',
	      example: '&oplus;',
	      button_groups: [
	        ["wedge", "vee", "diamond", "ominus", "uplus", "otimes", "oplus", "bigtriangledown", "sqcap"],
	        ["lfloor", "rfloor", "lceil", "rceil", "slash"],
	        ["triangleleft", "sqcup", "triangleright", "odot", "bigcirc", "dagger", "ddagger", "wr", "amalg"]
	      ]
      }
  ];
  ...
  var html_template_overrides = {
    binomial: '<span style="font-size: 1em; margin-top: -5px;"><span class="paren" style="font-size: 2.087912087912088em; ">(</span><span class="array"><span><var>n</var></span><span><var>m</var></span></span><span class="paren" style="font-size: 2.087912087912088em; ">)</span></span>',
    frac: '<span style="font-size: 1.2em; margin-top: -7px;" class="fraction"><span class="numerator"><var>n</var></span><span class="denominator"><var>m</var></span><span style="width:0"></span></span>',
    sqrt: '<span style="font-size: 1em; padding-top: 3px"><span class="sqrt-prefix">&radic;</span><span class="sqrt-stem" style="border-top-width: 1.7142857142857144px;">&nbsp;</span></span>',
    nthroot: '<span style="font-size: 1em"><sup class="nthroot"><var>n</var></sup><span><span class="sqrt-prefix">&radic;</span><span class="sqrt-stem" style="border-top-width: 1.7142857142857144px; ">&nbsp;</span></span></span>',
    supscript: '<sup style="font-size: 1em">sup</sup>',
    subscript: '<sub style="font-size: 1em; line-height: 2.5;">sub</sub>',
    vector: '<span class="array" style="font-size: 1em; margin-top: -3px;"><span class=""><var>a</var><span> </span><var>b</var></span><span class=""><var>c</var><span> </span><var>d</var></span></span>'
  }

  var tabs = [];
  var panes = [];
  jQuery.each(button_tabs, function(index_tab, tab){
		   if(index_tab==0){ //提示内容
			   tabs.push('<li class="mathquill-tab-selected"><a if="#' + tab.name + '_tab_'+index_pane+'"><span class="mathquill-rendered-math">' + tab.example + 
						  '</span>' + tab.name + '</a></li>');
		    }else{
		    	tabs.push('<li><a if="#' + tab.name + '_tab_'+index_pane+'"><span class="mathquill-rendered-math">' + tab.example + 
		  			      '</span>' + tab.name + '</a></li>');
		    }
		    var buttons = [];
		    jQuery.each(tab.button_groups, function(index_btn, group) {
		    	if(index_tab==0){
		    		buttons.push('<div class="btn-group mathquill-space"><a class="mathquill-rendered-math btn btn-default" title="' + group + '"><span id="ff" style="font-size: 1.3em;line-height: 1.4em">'+ group +'</span></a></div>');
		    	}else{
		    	  var btngroup = [];
		  	      jQuery.each(group, function(index_group, cmd) {
		  	        var obj = new LatexCmds[cmd](cmd, cmd);
		          	btngroup.push('<a class="mathquill-rendered-math btn btn-default" title="' + (cmd.match(/^[a-z]+$/) ? '\\' + cmd : cmd) + '">' +
		          			(html_template_overrides[cmd] ? html_template_overrides[cmd] : '<span style="font-size: 1.3em;line-height: 1.4em">' + obj.htmlTemplate + '</span>') +
//		                      (html_template_overrides[cmd] ? html_template_overrides[cmd] : '<span style="font-size: 1.3em;line-height: 1.4em">ddr400</span>') +
		                      '</a>');
		  	      });
		  	      buttons.push('<div class="btn-group mathquill-space">'+btngroup.join('')+'</div>');
		    	}
		    });
		    if(index_tab==0){//提示内容
		    	panes.push('<div class="mathquill-tab-pane mathquill-tab-pane-selected" id="' + tab.name + '_tab_'+index_pane+'">' + 
			    		buttons.join('') + '</div>');
			}else{
		    	panes.push('<div class="mathquill-tab-pane" id="' + tab.name + '_tab_'+index_pane+'">' + 
			    		buttons.join('') + '</div>');
		    }
	  });
	
  var toolbar = jQuery('<div class="mathquill-toolbar"><ul class="mathquill-tab-bar">' + tabs.join('') + //mathquill-tab-bar
		  '</ul><div class="mathquill-toolbar-panes">' + panes.join('') + 
		  '</div></div>');//.prependTo(jQ);
	toolbar.find('.mathquill-tab-bar li a').mouseup(function() {
		var parentLi = $(this).parent();
		parentLi.siblings("li").removeClass('mathquill-tab-selected');
		parentLi.parent().siblings(".mathquill-toolbar-panes").children(".mathquill-tab-pane").removeClass('mathquill-tab-pane-selected');
		
		parentLi.addClass('mathquill-tab-selected');
		jQuery(jQuery(this).attr("if")).addClass('mathquill-tab-pane-selected');
		root.jQ.focus();
	});
//	alert(toolbar.find('.mathquill-tab-bar li:first-child a').parent().html());
//	toolbar.find('.mathquill-tab-bar li:first-child a').trigger("mouseenter");
	//选择事件
	toolbar.find('a.mathquill-rendered-math').mousedown(function(e) {
	  	e.stopPropagation();
	}).click(function(){
		root.cursor.writeLatex(this.title);
//		var dd = jQ.find('span[class$="empty"]');
//		//dd.removeClass('empty');
//		dd.addClass('hasCursor');
		jQ.focus();
	});
	var popoverCss = "min-width:"+(jQ.width()+10)+"px;margin-top:-1px;" +
		"-webkit-box-shadow: none;box-shadow: none;" +
		"border-radius: 0px;border-top-right-radius: 3px;border-top-left-radius: 3px;";

	var popoverTemplate = "<div class='popover' style='"+popoverCss+"'>"+
			"<div class='popover-inner'><div class='popover-content' style='padding: 0px 2px;margin-top:-5px;'><p></p></div></div></div>";
	jQ.popover({
		html:true,
	...
	var jQId = jQ.attr("id");
	popoverStatus[jQId] = false;
	jQ.click(function(){
		if($(this).find(".cursor").length > 0){
			var editorId = $(this).attr("id");
			if(!popoverStatus[editorId]){
				$(this).popover("show");
				popoverStatus[editorId] = true;
			}
		}
	});
	jQuery("body").unbind("click");
	jQuery("body").bind("click",function(){
		//$('#ff').mathquill('editable');
		jQuery(".mathquill-editor").each(function(){
			if($(this).find(".cursor").length == 0){
				$(this).popover("hide");
				popoverStatus[$(this).attr("id")] = false;
			}
		});
	});
  index_pane++;
...
var Cursor = P(Point, function(_) {
  ...
  _.writeLatex = function(latex) {
	  ...
	  switch (latex){
		  case '\\frac':latex = '\\frac{}{}'; break;
		  case '\\binomial':latex = '\\binom{}{}'; break;
		  case '\\sqrt':latex = '\\sqrt{}'; break;
		  case '\\nthroot':latex = '\\sqrt[]{}'; break;
		  case '\\vector':latex = '\\vector{}'; break;
		  case '\\langle':latex = '\\langle{}'; break;
		  case '\\subscript':latex = '_{}'; break;
		  case '\\supscript':latex = '^{}'; break;
      }
    var self = this;
...
jQuery.fn.mathquill = function(cmd, latex) {
  ...
  default:
    ...
    return this.each(function() {
      createRoot($(this), RootBlock(), textbox, editable,include_toolbar);
    });
  }
};
