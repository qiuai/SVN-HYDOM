...
function tsF(root){
	  //提示内容
		//获取提示数据
		data = getTips(root.jQ);
		//动态修改提示内容
	    var tsObject = jQuery('div[class="popover fade bottom in"]').find('div[id^="提示内容_tab_"]');
	    var div = '';
	    if(data!=null&&data!=""){
	    	  for(var i=0;i<data.length;i++){
	   	       div += '<a class="mathquill-rendered-math2" title="'+ data[i] + '"><div class="btn-group mathquill-space btn btn-default"><span id="ts'+i+'">'+i+'、$'+ data[i] + '$</span></div></a>'
	   	    }
	   	    tsObject.html(div);
...
var RootMathBlock = P(MathBlock, function(_, _super) {
  ...
  _.onKey = function(key, e) {
    ...
    case 'Esc':break;
    case 'Tab':
    	this.jQ.parent().next("div").find("span").focus();
    	this.jQ.parent().next("div").find("span").popover("show");
    	if(this.jQ.parent().next("div").find("span").length==0){
    		$(this.jQ).parent().parent().next("div").find('span[id$="recog_0"]').focus();
    		$(this.jQ).parent().parent().next("div").find('span[id$="recog_0"]').popover("show");
    	}
...
//    		//root2.cursor = Cursor(root2);
    	...
    	if(jj.length==0){
    		...
    		$(this.jQ).after("<span class='mathquill-editor' style='padding-top:15px;height:45px;width:550px;' id='" + pid +"'></span>");
//    		var newRowHtml = "<div class='row stuAnswer' style='margin-right:0px;margin-left:0px;'>"+
//							"<div class='track'><svg class='recog' width='490' height='45'></svg></div>"+
//							"</div>";
    		//$(this.jQ).parent().prev().children().last().after(newRowHtml);
    		$("#" +pid).mathquill('editor');
    		//root2 = RootMathBlock();
    		//this.jQ = $("#" +pid)
		    	...
		    	if(index_tab==0){
		    	  ...
		    	  var btngroup = [];
		  	      jQuery.each(group, function(index_group, cmd) {
		          	...
		          	btngroup.push('<a style="text-decoration:none; color:#0000FF" class="mathquill-rendered-math btn btn-default" title="' + (cmd.match(/^[a-z]+$/) ? '\\' + cmd : cmd) + '">' +
