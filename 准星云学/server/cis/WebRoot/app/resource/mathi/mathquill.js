...
var MathCommand = P(MathElement, function(_, _super) {
  ...
  /*2015-04-01 add 主要实现不换行功能*/
  window.fixedWidth = 0;
  var checkHight = {
    after:function(offset){
      if(!window.fixedWidth){
        window.fixedWidth = $("#sizeChange").width();
      }
      var resize = window.fixedWidth;
      var heightRoot = $("#mathquill-editor");
      var last = heightRoot.find("span");
      var lastoff = last.eq(last.length -2).position();
      //自动增长
      if(offset.left >  resize * 0.8 || lastoff.left > resize * 0.8){
        heightRoot.width(heightRoot.width() *1.2);
      }
      //自动缩短
      if(heightRoot.width() > resize && lastoff.left < heightRoot.width()*0.6){
        if(lastoff.left *1.2 > resize){
          heightRoot.width(lastoff.left *1.2);
        }else if(lastoff.left < resize){
          heightRoot.width(resize);
        }else if(heightRoot.width() * 0.8> resize && heightRoot.width() * 0.8 > lastoff.left){
          heightRoot.width(heightRoot.width()*0.8);
        }
      }
    },
    after2:function(){
      //滚动条位置
      var resize = $("#sizeChange");
      for(var i=0;i<3;i++){
        if(offset.left > resize.width()/2){
          resize.scrollLeft(offset.left - resize.width()*2/3);
        }else{
          resize.scrollLeft(offset.left);
        }
      }
    }
  };
  ...
  _.createLeftOf = function(cursor) {
    ...
    //20150401
    checkHight.after(cursor.jQ.position());
  };
...
function createRoot(jQ, root, textbox, editable,include_toolbar) {
  ...
  jQ.bind('selectstart.mathquill', function(e) {
    ...
    //drag-to-select event handling for mobile
    var anticursor4sm, blink4sm = cursor.blink;
    jQ.bind('touchstart.mathquill', function(e) {
      ...
      function mousemove(e) {
        var touch = getMobileTouch();
        cursor.seek($(e.target), touch.pageX, touch.pageY);

        if (cursor[L] !== anticursor4sm[L]
            || cursor.parent !== anticursor4sm.parent) {
          cursor.selectFrom(anticursor4sm);
        }
      ...
      function mouseup(e) {
        //root2 = root;
        anticursor4sm = undefined;
        cursor.blink = blink4sm;
        if (!cursor.selection) {
          if (editable) {
            cursor.show();
          }
...
//mark code handle
      ...
      anticursor4sm = Point(cursor.parent, cursor[L], cursor[R]);
