...
public class CachedManager {
	 ...
	 * @param list - 待缓存集合
	 */
	public synchronized static <T extends BaseCache> void cacheAll(String cacheName, Collection<T> list) {
		Cache cache = getCache(cacheName);
		if (cache == null) {
			//log.warn("Do not find cache, named by [" + cacheName + "]");
			return;
		...
		for (T entity : list) {
			try {
				...
				//log.error("putcache(" + cacheName + ") of " + key + "failed.", e);
			}
		}
	}
	 ...
	 * @param list - 待缓存集合
	 */
	public synchronized static <T extends BaseCache> void cacheAll(List<T> list) {
	 ...
	 * @param entity - 待缓存实体
	 */
	public static <T extends BaseCache> void putObjectCached(T entity) {
		String cacheName = entity.getClass().getName();
		putObjectCached(cacheName, entity.getCacheKey(), entity);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T extends BaseCache> List<T> getAll(Class<T> clazz) {
	 ...
	 * @param entity - 待移除的实体
	 */
	public static void remove(BaseCache entity) {
