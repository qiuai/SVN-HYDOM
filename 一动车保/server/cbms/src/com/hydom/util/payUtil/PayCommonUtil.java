...
public class PayCommonUtil {
	 ...
	 * @return
	 */
	public String saveServiceOrder(String confimId, String tradeNum,
			String total_fee, Integer type) {
		OrderService orderService = (OrderService) SpringUtil
				.getBean("orderService");
		MemberCouponService memberCouponService = (MemberCouponService) SpringUtil
				.getBean("memberCouponService");
		FeeRecordService feeRecordService = (FeeRecordService) SpringUtil
				.getBean("feeRecordService");
		ProductService productService = (ProductService) SpringUtil
				.getBean("productService");
		MemberService memberService = (MemberService) SpringUtil
		...
		try {
			...
			// BigDecimal fb = new BigDecimal(0).setScale(2,
			...
			/*
			 * if(feeRecord == null){ feeRecord = new FeeRecord(); }
			 */
			feeRecord.setType(2);
			 ...
			 * feeRecordService.update(feeRecord); }else{
	 ...
	 *            交易金额
	 * @param type
	 *            交易类型 消费方式 比如 支付宝 银联 1=会员卡支付；2=支付宝 ；3=银联； 4=微信 5 现金支付
	 * @return
	 */
	public String saveRecharge(String out_trade_no, String trade_no,
			String total_fee, Integer type) {
		MemberService memberService = (MemberService) SpringUtil
				.getBean("memberService");
		FeeRecordService feeRecordService = (FeeRecordService) SpringUtil
				.getBean("feeRecordService");
		RechargeBenefitsService benefitsService = (RechargeBenefitsService) SpringUtil
				.getBean("rechargeBenefitsService");
		MemberCouponService couponService = (MemberCouponService) SpringUtil
				.getBean("memberCouponService");
		log.info("充值编号：" + out_trade_no + "进入充值保存流程");
		try {

			FeeRecord feeRecord = feeRecordService
					.findByRechargeNum(out_trade_no);
			...
			if (feeRecord.getType() == 3) {// 购买会员卡
				log.info("购买会员卡：");
				CouponPackageRecordService couponPackageRecordService = (CouponPackageRecordService) SpringUtil
						.getBean("couponPackageRecordServiceBean");
				feeRecord.setVisible(true);
				feeRecordService.update(feeRecord);
				couponPackageRecordService.returnCoupon(feeRecord);
				return "success";
			}
			...
			Float fanxian = 0f;
			List<RechargeBenefits> benefits = benefitsService
					.getListByMoney(total_fee);
				...
				if (benefits2.getType() == 1) {// 福利类型：1=充值返现；2=充值送优惠券
					...
					if (benefits2.getCouponNumber() != null) {
						...
						for (long i = 0l; i < benefits2.getCouponNumber(); i++) {
							...
							memberCoupon.setIntroduction(coupon
									.getIntroduction());
	 ...
	 *            商家退款交易号
	 */
	public String refundFeeRecord(String tradeNo, String price, String status,
			String batch_no) {
		...
		if ("SUCCESS".equals(status)) {
			FeeRecord feeRecord = feeRecordService
					.findByHql("select o from com.hydom.account.ebean.FeeRecord o where o.tradeNo = '"
							+ tradeNo + "'");
