...
import com.hydom.util.Setting.WatermarkPosition;
	...
	public static void zoom(File srcFile, File destFile, int destWidth,
		...
		if (type == Type.jdk) {
			...
			try {
				...
				imageWriteParam
						.setCompressionQuality((float) (DEST_QUALITY / 100.0));
				imageWriter.write(null, new IIOImage(destBufferedImage, null,
		...
		if (type == Type.jdk) {
			...
			try {
				...
				graphics2D.setComposite(AlphaComposite.getInstance(
						AlphaComposite.SRC_ATOP, alpha / 100F));
