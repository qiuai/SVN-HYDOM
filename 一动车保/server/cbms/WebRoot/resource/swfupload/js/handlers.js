...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("您上传的文件数超过限制.\n" + (message === 0 ? "You have reached the upload limit." : "您只能选择 " + (message > 1 ? "up to " + message + " files." : "一个文件.")));
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			progress.setStatus("无法上传大小为0 byte的文件.");
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		...
		//console.log(this.getStats().files_queued);
...
function load(){
	...
	$("body").append('<div id="progressBar" class="progressBar" style="display: none; "><img src="../../template/common/images/loading_bar.gif" /></div>');
