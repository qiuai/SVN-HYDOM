	...
	function addArea(){
		var div = 
			"<div class='form-group'>"+
				"<label class='col-sm-4 control-label'><a onclick='delArea(this)' style='float:left;'>删除</a> 地区"+(i+1)+"</label>"+
				...
				<div class="contentpanel">
					<div class="panel panel-default">
						...
						<form class="form-horizontal form-bordered" id="inputForm"
							action="<%=basePath%>manage/carTeam/update" method="POST">
							<input type="hidden" value="${carTeam.id }" name="id" id="id">
							<div class="panel-body nopadding" id="infoDiv">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type="text" class="form-control" name="headMember" CHname="负责人" 
											onBlur="check(this)" placeholder="请填写负责人姓名" id="headMember" value="${carTeam.headMember }">
										<input type="hidden" class="repeat"/>
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type="text" class="form-control" name="headPhone" CHname="电话" maxlength="11"
											onBlur="inputRestrict(this)" placeholder="请填写负责人电话" id="headPhone" value="${carTeam.headPhone }">
										<input type="hidden" class="repeat"/>
								...
								<div class="form-group">
									...
									<div class="col-sm-8">
										<input type="text" class="form-control" name="remark" placeholder="请填写备注" value="${carTeam.remark }">
									</div>
								</div>
								...
								<c:forEach var='area' items='${carTeam.area }' varStatus='index'>
									<div class='form-group'>
										<label class='col-sm-4 control-label'><a onclick='delArea(this)' style='float:left;'>删除</a> 地区${index.index+1 }</label>
										<div class='col-sm-8' id='area_div_select'>
											<c:if test="${area.parent.parent != null }">
												<label class='control-label'>${area.parent.parent.name }</label>
											</c:if>
											<c:if test="${area.parent != null }">
												<label class='control-label'>${area.parent.name }</label>
											</c:if>
											<label class='control-label'>${area.name }</label>
										</div>
										<input type='hidden' class='areaSelectId' name='area[${index.index }].id' value='${area.id }'/>
										<input type="hidden" class="repeat"/>
