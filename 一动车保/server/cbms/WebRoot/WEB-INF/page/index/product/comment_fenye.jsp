...
<div style="margin-top: 10px;margin-bottom: 10px;position: absolute;bottom: 0px;right: 0px">
	<div style="float: left;height: 32px; margin-top: 6px;">
		<ul class="pagination  pagination-md" style="margin: 0px;"> 
			...
			<c:if test="${(pageView.currentPage-1)>=1}" >
				<li> <a href='javascript:toCommentPage(${pageView.currentPage-1})' >Prev</a> </li>
			</c:if>
			<li> <a href='javascript:toCommentPage(1)' class="re" ${pageView.currentPage==1?'style="background-color: #efefef;"':''} >1</a> </li>
			<c:if test="${pageView.pageIndex.startindex>2}" >
	 		...
	 		<c:forEach begin="${pageView.pageIndex.startindex}" end="${pageView.pageIndex.endindex}" var="per">
				<li> <a href='javascript:toCommentPage(${per})' ${pageView.currentPage==per?'style="background-color: #efefef;"':''}>${per}</a> </li>  
			</c:forEach>
			...
			<c:if test="${pageView.totalPage>=2}">
				<li> <a href='javascript:toCommentPage(${pageView.totalPage})' ${pageView.currentPage==pageView.totalPage?'style="background-color: #efefef;"':''} >${pageView.totalPage}</a> </li>
			</c:if>
			...
			<c:if test="${(pageView.currentPage+1)<=pageView.totalPage}" >
				<li > <a href='javascript:toCommentPage(${pageView.currentPage+1})' >Next</a> </li>
			</c:if>
		</ul>
	</div>
