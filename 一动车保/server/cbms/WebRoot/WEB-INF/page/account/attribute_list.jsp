    ...
    <div class="mainwrapper">
		<%@ include file="/WEB-INF/page/common/left.jsp" %>
         <div class="mainpanel">
		    ...
		    <form id="pageList" action="${pageContext.request.contextPath}/manage/productAttribute/list" method="post">
			    <div class="contentpanel">
			      <div class="search-header">
			        <div class="btn-list">
			          <button class="btn btn-danger" id="deleteButton" type="button" val="<%=path %>/manage/productAttribute" disabled>删除</button>
			          <button class="btn btn-success" id="refreshButton">刷新</button>
					  <button id="add" type="button" class="btn btn-primary" val="<%=path %>/manage/productAttribute">添加</button>
			      ...
			      <div class="table-responsive">
			        <table id="listTable" class="table table-info" >
						...
						<c:forEach items="${pageView.records}" var="attribute" >  
							<tr>
								<td>
								...
								<td>
									<c:forEach var="option" items="${attribute.options }" begin="0" end="5">
										${option }&nbsp;&nbsp;
									</c:forEach>
									<c:if test="${attribute.options.size()>6 }">
										...
									</c:if>
								</td>
								<td>
								...
								<td>
									<a href="<%=base %>manage/productAttribute/edit?id=${attribute.id}">[编辑]</a>
								</td>
							</tr>
						  </c:forEach>
					</table>
