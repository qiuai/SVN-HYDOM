...
function checkMoney() {
	...
	if(money == "" || money == null){
		...
		.post(
				...
				function(data) {
				 				if (data == true && money!="${rechargeBenefits.money }") {
	...
	var scale = $("#scale").val();
		...
		if ($("#scale").is(":visible")) {
			if (scale == "" || scale == null) {
				...
				$("#scale_error").html("请输入正确的返现率(必须为正数)，最多保留小数点后两位");
				$("#scale").next().next().val("");
			}
			else{
				$("#scale_error").html("");
				...
				<div class="contentpanel">
					<div class="panel panel-default">
						...
						<form class="form-horizontal form-bordered" id="inputForm"
							action="<%=basePath%>manage/rechargebenefits/update" method="POST">
							<input type="hidden" name="id" value="${rechargeBenefits.id }">
							<input type="hidden" id="rType" value="${rechargeBenefits.type }">
							<div class="panel-body nopadding">
								<div class="form-group">
									<label class="col-sm-4 control-label">充值金额</label>
									<div class="col-sm-8">
										<input type="text" class="form-control" name="money"
										 onBlur="checkMoney()" placeholder="请填写充值金额" id="money" value="${rechargeBenefits.money }">
								...
								<div class="form-group">
										...
										<div class="col-sm-8">
											<select id="type" name="type" class="selects" onchange="selectType(this)">
												<option value="1" id="sm" selected="selected">充值返现</option>
												<option value="2" id="sc">充值送优惠券</option>
											</select>
										</div>
									</div>
								...
								<div class="form-group" id="three" >
									...
									<div class="col-sm-8" style="position: relative;">
										<input type="text" class="form-control" name="scale"
										 onBlur="checkScale()" placeholder="请输入返现比例，如：50%就输入0.5" id="scale" value="${rechargeBenefits.scale }">
								...
								<div class="form-group" id="one" style="display: none">
									...
									<div class="col-sm-8">
										<select id="useTypeId" name="useTypeId" class="selects" onchange="useTypeChange(this);">
											<option value="1" <c:if test="${rechargeBenefits.coupon.useType==1 }">selected="selected"</c:if>>洗车优惠券</option>
											<option value="2" <c:if test="${rechargeBenefits.coupon.useType==2 }">selected="selected"</c:if>>保养优惠券</option>
											<option value="3" <c:if test="${rechargeBenefits.coupon.useType==3 }">selected="selected"</c:if>>商品优惠券</option>
										</select>
									</div>
								</div>
								...
								<div class="form-group" id="two" style="display: none">
									...
									<div class="col-sm-8">
										<select id="coupon" name="coupon.id" class="selects">
											...
											<option value="${coupon.id }" <c:if test="${rechargeBenefits.coupon.id==coupon.id }">selected="selected"</c:if>>${coupon.name }</option>
											</c:forEach>
										</select>
									</div>
								</div>
								...
								<div class="form-group" id="four" style="display: none">
									...
									<div class="col-sm-8">
										<input type="text" class="form-control" name="couponNumber"
											onBlur="checkNum()" placeholder="请填写优惠券数量，不能为负数" id="couponNumber" value="${rechargeBenefits.couponNumber }">
										<input type="hidden" class="repeat"/>
