    ...
    <div class="mainwrapper">
		<%@ include file="/WEB-INF/page/common/left.jsp" %>
         <div class="mainpanel">
		    ...
		    <form id="pageList" action="${pageContext.request.contextPath}/manage/serviceStore/list" method="post">
			    <div class="contentpanel">
			      <div class="search-header">
			        <div class="btn-list">
			          <button class="btn btn-danger" id="deleteButton" type="button" val="<%=path %>/manage/serviceStore" disabled>删除</button>
			          <button class="btn btn-success" id="refreshButton">刷新</button>
					  <button id="add" type="button" class="btn btn-primary" val="<%=path %>/manage/serviceStore">添加</button>
			      ...
			      <div class="table-responsive">
			        <table id="listTable" class="table table-info" >
						...
						<c:forEach items="${pageView.records}" var="store">  
							<tr>
								<td>
								...
								<td>
									<c:forEach var="serviceType" items="${store.serviceTypeSet }">
										<span>${serviceType.name }</span> &nbsp; 
									</c:forEach>
								</td>
								<td>
									<span><fmt:formatDate value="${store.createDate}" pattern="yyyy-MM-dd"/></span>
								</td>
								<td>
									<a href="<%=base %>manage/serviceStore/edit?id=${store.id}">[编辑]</a>
								</td>
							</tr>
						  </c:forEach>
					</table>
