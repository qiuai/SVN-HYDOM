		      ...
		      <div class="media">
		        <div class="media-body">
		          <ul class="breadcrumb">
		            ...
		            <li><a href="">服务类型列表</a></li>
		          </ul>
		        </div>
		      </div><!-- media -->
		    </div>
		    <form id="pageList" action="${pageContext.request.contextPath}/manage/serviceType/list" method="post">
			        ...
			        <table id="listTable" class="table table-info" >
						...
						<c:forEach items="${pageView}" var="services" varStatus="s">  
							<tr>
								...
								<td>
									<span><fmt:formatDate value="${services.createDate}" pattern="yyyy-MM-dd"/></span>
								</td>
								<td>
									<span><fmt:formatDate value="${services.modifyDate}" pattern="yyyy-MM-dd"/></span>
								</td>
								<td>
									<a href="<%=base %>manage/serviceType/edit?id=${services.id}">[编辑]</a>
								</td>
							</tr>
						  </c:forEach>
					</table>
