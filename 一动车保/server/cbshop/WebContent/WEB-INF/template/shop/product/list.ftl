								...
								[#list row as product]
									<li>
										<a href="${base}${product.path}">
											<img src="${base}/upload/image/blank.gif" width="170" height="170" data-original="[#if product.image??]${product.image}[#else]${setting.defaultThumbnailProductImage}[/#if]" />
											<span class="price">
												<div>${currency(product.price, true)}</div>
												<div>
