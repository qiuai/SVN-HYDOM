...
- (void)didTouch
{
    if ([self dateIsBetweenDateAndEndDate])
    {
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if(selected){
        ...
        BOOL isSelectDay;
        if (self.calendarManager.beginDate!=nil&&self.calendarManager.endDate!=nil) {
            isSelectDay=[self dateIsBetweenDateAndEndDate];
        }else{
            isSelectDay=YES;
        }
        
        if(!self.isOtherMonth&&isSelectDay){//当前月
...
#pragma -mark 判断时间是否在两时间中间
-(BOOL)dateIsBetweenDateAndEndDate
{
    if ([self.date compare:self.calendarManager.beginDate] == NSOrderedAscending)
        return NO;
    if ([self.date compare:self.calendarManager.endDate] == NSOrderedDescending)
        return NO;
    return YES;
}
