...
#define kTakePhotoString LocalizedString(@"使用")
#define kSelectPhotoFromLibraryString LocalizedString(@"从相册选择图片")
#define kRecordVideoString LocalizedString(@"Record video")
#define kSelectVideoFromLibraryString LocalizedString(@"从相册选择视频")
...
//        [self showAlertController:view];//菜单栏
        [self takePhotoFromPhotoLibrary];//修改直接从相册获取
//    } else {
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if ([viewController isKindOfClass:NSClassFromString(@"PLUIImageViewController")] && self.editMode && [navigationController.viewControllers count] == 3) {
        ...
        position = (screenHeigth -screenWidth)/2;
